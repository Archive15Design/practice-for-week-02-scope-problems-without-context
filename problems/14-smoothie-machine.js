...
const smoothieMachine = (...ingredients) => {
  let smoothieArr = Array();
  return function(...moreIngredients){
    ingredients.forEach(ele => smoothieArr.push(ele));
    moreIngredients.forEach(ele => smoothieArr.push(ele));

    let smoothieStr = smoothieArr.join(" and ");

    return "I'm having a smoothie with " + smoothieStr;
  }
};
